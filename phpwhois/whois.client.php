...
class WhoisClient {
	...
	*/
	...
	function Process (&$result, $deep_whois=true) {
		...
		// Process
		// $res = $handler->parse($result,$this->Query['query']); // original line (send string Query['query'] intstead of array Query, which the parse-function tries to access)
		$res = $handler->parse($result,$this->Query); // my corrected line -- everdimension
